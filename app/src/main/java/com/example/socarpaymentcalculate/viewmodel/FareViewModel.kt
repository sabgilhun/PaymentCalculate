...
class FareViewModel(private val repository: TmapRepository) : BaseViewModel() {
    ...
    fun onSelectCarType(selectedCarType: CarType) {
    ...
    fun onSelectCarModel(selectedCarModel: CarModel) {
    ...
    private fun calculateFare() {
        if (route != null && _selectedCarModel.value != null) {
            _calculatedFare.value =
                ((route!!.totalDistance / 1000.0) * _selectedCarModel.value!!.farePerKiloMeter).toInt().toString()
        }
    }
